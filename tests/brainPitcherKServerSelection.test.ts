...
function snapshot(index: number, overrides: Partial<BrainFeatureSnapshot> = {}): BrainFeatureSnapshot {
  return {
    ...
    adapterVersion: "mlb-pitcher-k-features@1", quality: "limited", eligibility: "preview",
...
describe("server pitcher K selection", () => {
  it("selects at most one probable pitcher per game and keeps preview honesty", () => {
    ...
    expect(selected.every((item) => item.snapshot.eligibility === "preview")).toBe(true);
  });
  ...
  it("applies a preview penalty versus eligible probable labels", () => {
    const preview = selectMlbPitcherKFeatures([snapshot(1)], new Date("2026-07-12T17:00:00.000Z"));
    const eligible = selectMlbPitcherKFeatures(
      [snapshot(1, { eligibility: "eligible" })],
      new Date("2026-07-12T17:00:00.000Z"),
    );
    expect(preview[0]?.score).toBeLessThan(eligible[0]?.score ?? 0);
  });
});
