...
import { buildResultsAuroraSummary } from '../src/components/results/resultsAuroraModel';
...
describe('Aurora results presentation', () => {
  it('computes record counts without inventing financial performance', () => {
    const summary = buildResultsAuroraSummary([
  ...
  it('keeps win rate unavailable before any win or loss', () => {
    expect(buildResultsAuroraSummary([parlay({})]).winRate).toBeNull();
  });
