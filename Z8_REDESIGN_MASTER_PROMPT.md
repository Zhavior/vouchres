...
- **`a845049` — HR Intelligence: labeled simulated stats honestly, removed dead code**
  Surveyed the whole `src/features/hr/` feature (13 files, ~4000 lines) before touching anything. Real finding: `HrStatsTab.tsx` and `HrPlayerProfile.tsx` both fabricate batter-vs-pitcher career history, vs-team last-5-games, and last-10-games recent form via deterministic-seed generators, rendered next to real player names with zero disclaimer. `src/lib/mlbDirect.ts` already establishes the correct precedent for this exact situation (label unavailable data, never invent it) — real per-batter-vs-pitcher/per-game data isn't fetchable client-side at reasonable API cost, so removing the mockup outright wasn't a quick fix. Added a `simulated` prop to both files' section-header components that renders a persistent "Simulated" badge on all three fabricated sections. Also removed a decorative glow blob in `HrPlayerProfile.tsx` (the same gradient-blob pattern eliminated elsewhere), deleted two confirmed-dead files (`HrWatchCard.tsx`, `HrWatchTierSection.tsx` — orphaned, nothing renders them), and replaced two hardcoded hex backgrounds with the existing `--ve-bg-deep` token.
  **Investigated but not a bug**: a suspected `TIER_STYLES` key mismatch in `HrSpreadsheet.tsx` was a false positive — types match exactly, plus a safe fallback already exists. Worth remembering: verify a suspected bug against the actual type definitions before "fixing" it.
  **Not done**: this feature still uses the `--ve-*` token system (blue-hued), not a full Z8 (obsidian/emerald/cyan) conversion — deliberately scoped out given the size (4000+ lines); the tokens are at least consistent (not a rainbow), so it's lower priority than the fake-data fix.

  **Update — the app was externally restructured since the note above was written** (not by this assistant; per an in-session system note, the user/a linter changed `App.tsx`, `HomeFeedLayout.tsx`, `featureConfig.ts`). `isOpenEdgeDashboardMode`/`TheEdgeShell.tsx` no longer exist. `hasRealAuthToken()` (App.tsx) now correctly checks real Supabase `sb-*-auth-token` storage, not legacy keys — the old bug is gone. The `'welcome'` section now renders `EdgeIslandPage` (a new page at `src/pages/EdgeIslandPage.tsx`, composed from `src/components/edgeIsland/*`) for logged-in users. `TheEdgeShell.tsx` and its `WelcomeIntro.tsx` hero became fully orphaned in this restructuring.

- **`7a2b861` — Logout now shows a "Goodbye" screen and lands on a real Front Page**
  The restructuring above left no distinct public landing page — logged-out users (including right after logout) saw `EdgeIslandPage` with a small "Sign in to unlock..." banner bolted on, not a real marketing page. Built `src/pages/FrontPage.tsx` (fetches the real daily MLB slate, renders the previously-orphaned `WelcomeIntro.tsx` hero, wires CTAs to the existing `AuthModal` rather than reviving `TheEdgeShell`'s old bespoke signup wizard). `App.tsx`'s `'welcome'` case now branches: logged-in → `EdgeIslandPage`, logged-out → `FrontPage`. Added `GoodbyeScreen.tsx`, a ~900ms full-screen transition shown between clicking Logout and landing back on the front page (mirrors the existing post-signup "Welcome back" transition). Also deleted a stray byte-identical duplicate, `WelcomeIntro 2.tsx`.
  **Not verified end-to-end**: couldn't test the actual logout click in this environment (no real Supabase session available to log out of) — verified the code path by reading it, plus `tsc`/build clean and the Front Page itself confirmed live with real data.

- **`03cb464` — Route straight into Edge Island after signup when email confirmation is off**
  User clarified the intended journey: Front Page for everyone by default → Edge Island is "the first thing" logged-in/just-signed-up users see. Login already worked (calls `onAuthed()` → `handleLoginSuccess` → `navigateSection('welcome')` → `EdgeIslandPage` since a session now exists). Signup didn't: `AuthModal.tsx` always showed "Check your inbox" after `signUpWithEmail`, even when Supabase's `auth.signUp()` returns a live session immediately (happens when the Supabase project has email confirmation disabled) — the user would already be logged in but get stuck on a confirmation-email prompt for an email that isn't coming. Fixed by checking `data?.session` after signup: if present, call `onAuthed()` immediately (same route login uses); only show "check your inbox" when Supabase actually requires confirmation (`session` is null).
  **Not verified end-to-end**: didn't create a real test signup account in this environment to avoid writing live data into whatever Supabase project this points at — the fix follows the standard Supabase JS `data.session` check pattern, and reuses login's already-confirmed-working `onAuthed` route.

- **`6234575` — Upgraded the signup experience: intro slides + Pro/Capper plan picker with beta framing**
  `AuthModal.tsx` signup flow is now a 3-step wizard (login unchanged): (1) two intro slides with dot-nav/Next/Back/Skip, (2) a plan picker — Basic (free) / Pro ($12.99, Beta badge) / Capper ($49.99, Beta badge) — with persistent "signing up now helps support development" copy, (3) the existing account form, now with a "← {Plan} plan" back-link and submit copy reflecting the chosen plan. On successful signup with a paid plan selected, calls `startStripeCheckout('gold' | 'seller_pro')` (already existed in `billingClient.ts`) and redirects to Stripe before entering the app.

- **`87e853e` — Renamed Premium page to "Upgrade", matching Pro/Capper/Beta framing + a real chrome-visibility bug found along the way**
  `featureConfig.ts`'s "PRO Premium Tiers" nav label → "Upgrade". `PremiumSubPage.tsx`: removed a disconnected fake "Closed Beta" email-waitlist form (localStorage-only, not wired to anything), replaced with a plain beta-status banner; renamed "VEdge Gold"→"Pro" and "Research Seller PRO"→"Capper" to match AuthModal; added Beta badges. Underlying `SubscriptionTier` values and real Stripe logic untouched — copy/naming pass only.
  **Real bug found while testing this**: `HomeFeedLayout.tsx` was hiding the sidebar/mobile-header/right-rail/bottom-nav whenever `activeSection === 'welcome'` — correct back when `'welcome'` only ever showed the public front page, but since the earlier Front-Page-vs-Edge-Island fix, `'welcome'` also renders `EdgeIslandPage` for logged-in users, so they were losing the whole app shell on their own dashboard. Fixed with a new `isPublicFrontPage` prop (`activeSection === 'welcome' && !hasRealAuthToken()`) swapped in everywhere the layout used to key off `activeSection === 'welcome'` directly. **Worth remembering**: any future check gated on `activeSection === 'welcome'` in this codebase needs to account for the fact that section now serves two different pages depending on auth state.

- **`f7c36c3` — Rebuilt the Upgrade page on Z8 tokens**
  `PremiumSubPage.tsx` was still on the old hardcoded slate/sky/indigo/amber palette despite already being renamed to "Upgrade." Converted to `glass-panel`/`glass-border` + `vouch-emerald` (Pro tier) / `vouch-cyan` (Capper tier); kept amber only for "Beta" status badges (semantic exception, not part of the two-accent system). Presentation-only — Stripe logic and pricing untouched.

- **`4e00330` — Added a real policy agreement step to signup**
  Signup previously only had a passive one-line disclaimer at the form's bottom. Added a dedicated "Review & agree" step (`intro → plan → policy → form`): a scrollable box covering age/jurisdiction (21+), research/entertainment-only, no guaranteed returns, data storage, and billing terms — plus three required checkboxes that must all be checked before "Agree & continue" enables, structurally gating the account-creation form. Form step's back-link now returns to `policy` (was `plan`).
...
- **`89e28ae` — Fixed login never being detected (the "Edge Island not appearing after login" bug)**
  Root cause: commit `44ba2ef` gave the Supabase client a custom `storageKey: "vouchedge.auth"` (lib/supabaseClient.ts), but App.tsx's `hasRealAuthToken()` still scanned localStorage only for Supabase's DEFAULT `sb-<ref>-auth-token` key format — so a fully successful login was invisible to the router: EdgeIslandPage rendered its LoggedOutTeaser, reloads landed on the public FrontPage, `VouchEdgeBootGate` never enabled. Fix recognizes `vouchedge.auth` (+ legacy `sb-*` for old sessions) and no longer aborts the scan on one malformed key. Verified: guest path unchanged in-browser; side-by-side eval of old vs fixed logic against the real key format (old=false, fixed=true). **Full end-to-end still needs one real login by the user** — Supabase purges synthetic sessions on boot so it can't be fully simulated. **Worth remembering**: `vouchedge_after_auth_destination` / `vouchedge_after_auth_mode` are written on login but never read anywhere — dead keys; the post-login redirect never resumes a stored destination.

- **`d446845` — Live at-bat "sweat screen" (pitch-by-pitch view) in Live Games Center**
  User asked to replicate a Gameday-style live play screen (pitch sequence + strike-zone plot + spray chart + EV/LA/distance). All of it comes from MLB's free GUMBO feed (`/api/v1.1/game/{gamePk}/feed/live`: `pitchData.coordinates.pX/pZ` + per-batter `strikeZoneTop/Bottom`, `hitData.launchSpeed/launchAngle/totalDistance/coordinates`) plus the free `/winProbability` endpoint. **xBA is Statcast-only and deliberately NOT shown** — no faked stats. New: `server/services/mlb/liveAtBatService.ts` (slim snapshot, 12s TTL on top of existing `getGameFeed`), `GET /api/mlb/live-at-bat/:gamePk`, `src/types/liveAtBat.ts`, `src/components/live/LiveAtBatView.tsx` (Z8: emerald=in play, rose=strike, muted=ball, cyan=chrome; 15s poll), wired into `LiveGamesPro` for the selected live game. Verified against a real in-progress MIL@STL game end-to-end, including a 3-foot bunt single plotting correctly at home plate on the spray chart. Candidate follow-ups (not built): per-pick "watch my player" deep-link from HR board picks into this view; social chat layer via Supabase Realtime (skipped — viewer counts can't be faked honestly).

- **`933ce63` — Mobile nav overhaul: X-style profile drawer replaces the bottom bar**
  User asked: remove the mobile bottom nav, add a corner profile photo that opens a Twitter-like sidebar, with the avatar showing the real subscription tier. Done: `FeedMobileNav.tsx` deleted (14 icons never fit a phone); new `MobileProfileDrawer.tsx` slides in from the left off the header avatar. `tierMeta()`/`TierAvatar` (exported from the drawer file) map `profile.subscriptionTier` → visual identity: BASIC muted / GOLD (Pro) emerald / SELLER_PRO (Capper) cyan — used by both the header avatar ring and the drawer chip, and Capper accounts don't see the UPGRADE chip. Drawer nav items come from the same `featureConfig` registry as the desktop sidebar (no drift). Mobile header rebuilt on Z8 tokens. Verified at 375×812 end-to-end. **Worth remembering**: the dev/default profile resolves as SELLER_PRO, so the UPGRADE chip is hidden in dev — check with a BASIC profile before assuming the chip broke.
