...
- **`7c8e4e4` — Rebuild the entire FeedSidebar.tsx on Z8 Obsidian tokens**
  ...
  **Known open item (flagged, not fixed this pass)**: a 5-judge critique of the social feed (`HomeFeedPage.tsx`/`FeedComposer.tsx`) found a dead "Add image" button (`FeedComposer.tsx` ~line 241, no `onClick`), copy-pasted empty-state blocks, and gold/pink accents still in `HomeFeedPage.tsx` (PRO MODE toggle, verified badges) — separate pass, not yet started.
