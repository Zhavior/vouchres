...
export function HrNextKeyboardCheatsheet({ isOpen, onClose }: HrNextKeyboardCheatsheetProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 animate-in fade-in duration-150 font-mono">
      <div className="relative w-full max-w-md bg-black border-2 border-white/20 p-6 space-y-4">
        ...
        <div className="space-y-2 text-xs">
          {shortcuts.map((sc, idx) => (
            <div key={idx} className="flex items-center justify-between p-2.5 border border-white/10 bg-obsidian-950">
              <span className="text-white/70">{sc.desc}</span>
              <kbd className="px-2 py-0.5 border border-ve-cyan/50 bg-ve-cyan/50 text-ve-cyan font-bold text-[11px]">
                {sc.key}
              </kbd>
            </div>
          ))}
        ...
        <div className="text-[10px] text-white/40 text-center pt-3 border-t border-white/10">
          PRESS <kbd className="text-white/70 px-1 py-0.5 border border-white/20 bg-obsidian-800">[?]</kbd> ANYTIME TO TOGGLE KEYBINDINGS
        </div>
      </div>
    </div>
  );
