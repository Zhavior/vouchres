  ...
  return (
    <section className="px-4 md:hidden font-mono" aria-label="MLB tactical intel wire">
      <div className="flex items-center justify-between pb-2">
        ...
        <span className="text-[9px] text-white/40 uppercase">SWIPE STORIES →</span>
      </div>
        ...
        {items.map((item) => {
          ...
          return (
            <button
              ...
              className="w-[calc(100vw-3rem)] max-w-[300px] shrink-0 snap-start border border-white/[0.08] bg-[#0A0A0A] p-3 text-left flex flex-col justify-between space-y-2.5 transition-colors hover:border-white/[0.18] min-h-[140px] cursor-pointer rounded-none"
            >
              {/* Top Row: Category Tag & Timestamp */}
              <div className="flex items-center justify-between w-full">
                <span className={`px-1.5 py-0.5 text-[8px] font-mono font-medium uppercase border tracking-wider rounded-none ${style.pill}`}>
                  {style.label}
                </span>
                <span className="text-[8px] text-white/55 font-mono">{relativeTime(item.publishedAt)}</span>
              </div>

              {/* Headline */}
              <h4 className="text-[#ffffff] font-medium text-xs leading-snug line-clamp-2 font-sans">{item.headline}</h4>
              ...
              <div className="flex items-center justify-between w-full pt-1.5 border-t border-white/[0.06] text-[9px]">
                {mentions.length > 0 ? (
                  <span className="text-ve-emerald font-medium flex items-center gap-1">
                    <Flame className="h-2.5 w-2.5" /> {mentions.length} SLATE BAT{mentions.length > 1 ? 'S' : ''}
                  </span>
                ) : (
                  <span className="text-white/40 font-mono tracking-wider uppercase text-[8px]">TACTICAL INTEL</span>
                )}
                <span className="text-ve-cyan font-medium uppercase font-mono tracking-wider">READ →</span>
              </div>
            </button>
          );
        })}
