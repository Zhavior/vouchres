...
function calculateEdge(row: ChunkA): {
  modelProb: number | null;
  impliedProb: number | null;
  evEdge: number | null;
  oddsLabel: string | null;
} {
  const modelProb = row.score?.modelProbability ?? null;
  const impliedProb = row.odds?.impliedProbability ?? null;
  const rawEdge = (modelProb != null && impliedProb != null) ? (modelProb - impliedProb) * 100 : null;
  const evEdge = rawEdge == null ? null : Math.max(0, rawEdge);
  const oddsLabel = row.odds?.price ? (row.odds.price > 0 ? `+${row.odds.price}` : `${row.odds.price}`) : null;

  return { modelProb, impliedProb, evEdge, oddsLabel };
}

export function ChunkAEdgeDesk({ data }: Props) {
    ...
    return processedRows
      .filter(({ row, evEdge }) => {
        if (searchQuery.trim()) {
          ...
          const matchesName = row.identity.name.toLowerCase().includes(q);
          const matchesTeam = row.identity.teamAbbreviation.toLowerCase().includes(q);
          const matchesPitcher = row.opposingPitcherName.toLowerCase().includes(q);
      ...
      .sort((a, b) => {
        ...
        if (sortBy === "score") return b.row.score.hrIndex - a.row.score.hrIndex;
        if (sortBy === "prob") return compareDesc(a.modelProb, b.modelProb);
        if (sortBy === "odds") return compareDesc(a.row.odds?.price ?? null, b.row.odds?.price ?? null);
        return 0;
      });
  ...
  return (
    <section className="hr-edge-desk aurora-max-ranked-workspace space-y-4" data-workspace="edge">
        ...
        {stats.topEntry && (
          <div className="relative z-10 mt-6 flex flex-col items-center justify-between gap-4 rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-4 sm:flex-row sm:px-6">
            <div className="flex items-center gap-4">
              <PlayerHeadshot
                name={stats.topEntry.row.identity.name}
                mlbId={stats.topEntry.row.identity.mlbId}
                size={48}
              />
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-semibold text-white/60">
                    {stats.topEntry.row.identity.teamAbbreviation} vs {stats.topEntry.row.opponentTeamId}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <h4 className="text-lg font-black text-white">{stats.topEntry.row.identity.name}</h4>
                </div>
                <p className="text-xs text-emerald-200/80">
                  vs {stats.topEntry.row.opposingPitcherName} • HR Score: <strong className="text-white">{stats.topEntry.row.score.hrIndex}</strong>
                </p>
              </div>
            </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {filteredRows.map(({ row, modelProb, impliedProb, evEdge, oddsLabel }) => {
          const isPrime = evEdge != null && evEdge >= 8;
          const isPositive = evEdge != null && evEdge > 0;
          const logoUrl = logoByTeamName(row.identity.teamAbbreviation);
            ...
            >
              <div>
                ...
                <div className="mt-4 grid grid-cols-2 gap-2 text-center">
                  <div className="rounded-xl border border-white/5 bg-white/[0.02] p-2.5">
                    ...
                    <div className="font-mono text-lg font-black text-white">{row.score.hrIndex}</div>
                  </div>
              ...
              <div className="mt-5 flex items-center justify-between border-t border-white/10 pt-3.5">
                <div className="flex items-center gap-1.5 font-mono text-[10px] text-white/40">
                  ...
                  <span>{row.lineupStatus === "confirmed_starter" ? "Official Lineup" : "Projected"}</span>
                </div>
