...
import {
  ...
  presentWatchRow,
  sortIntelV2Rows,
  type IntelV2DisplayTier,
  type IntelV2Row,
  type IntelV2SortKey,
} from '../../hr/presentWatchRow';
...
function nextSort(current: IntelV2SortKey): IntelV2SortKey {
...
function downloadReceipts(rows: IntelV2Row[]) {
  const blob = new Blob(
    [
      JSON.stringify(
        rows.map((row) => ({
          ...
          strike:         row.strikeLine,
          hrpiLine:       row.hrpiLine,
          tier:           row.displayTier,
          hrpi:           row.score,
          lineup:         row.lineupLabel,
          signal:         row.signal,
          read:           row.read,
          matchupSummary: row.matchupSummary,
...
function TierColumn({
  ...
  const copy    = INTEL_V2_TIER_COPY[tier];
...
export default function AuroraHqDesk() {
  ...
  const [sortKey,       setSortKey]       = useState<IntelV2SortKey>('hrpi');
  ...
  const presented   = useMemo(
    () => vm.rows.map((row) => presentWatchRow(row, vm.slate.freshness, vm.slate.generatedAt, boardSource)),
    [boardSource, vm.rows, vm.slate.freshness, vm.slate.generatedAt],
  );
  const visibleRows = useMemo(() => sortIntelV2Rows(presented, sortKey), [presented, sortKey]);

  const boardBuckets = useMemo(() => {
    const buckets: Record<IntelV2DisplayTier, IntelV2Row[]> = { Elite: [], Strong: [], Watch: [], Sleepers: [] };
  ...
  /* ── Active row auto-fix ── */
  useEffect(() => {
    if (!activeRow) { setActiveId(null); return; }
    if (!visibleRows.some((r) => r.id === activeId)) setActiveId(activeRow.id);
  ...
  const addToSlip = (id: string) => {
    ...
    if (!row || !row.canAddToSlip) return;
    setSavedIds((curr) => new Set(curr).add(id));
    openParlayAdd({
      player:           row.player,
      propHint: {
        id:      `aurora-hq-${row.stableId}`,
        market:  'Home Runs',
        odds:    row.bookOdds,
