...
export const StatPlayerCard: React.FC<Props> = ({ row, statType, onClick }) => {
  ...
  return (
    <button
      type="button"
      onClick={() => onClick(row)}
      className="ve-stat-player-card ve-stat-surface group relative flex w-full flex-col gap-2.5 rounded-xl border border-white/10 p-3.5 text-left transition hover:border-white/20 focus-visible:outline focus-visible:outline-2 focus-visible:outline-white/30"
