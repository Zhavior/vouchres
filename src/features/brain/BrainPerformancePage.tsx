...
function MarketSummary({ label, target, market }: { label: string; target: string; market?: MarketLedger }) {
  ...
  return (
    <article className="brain-market-card border border-white/10 bg-black/40 p-4 space-y-3">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="text-right">
          <strong className="font-mono text-2xl font-bold text-vouch-emerald">
            {performance.hitRate == null ? '—' : `${performance.hitRate}%`}
          </strong>
          <span className="block text-[9px] font-mono text-white/55 uppercase font-bold">Hit Rate</span>
        </div>
      </div>
      <div className="grid grid-cols-3 gap-2 border-t border-white/10 pt-3 text-center font-mono text-xs">
        <div className="p-2 bg-white/5">
          <span className="text-[9px] uppercase text-white/55 block font-bold">Recorded</span>
          <strong className="text-white font-bold text-sm">{performance.total}</strong>
        </div>
        <div className="p-2 bg-vouch-emerald/10 border border-vouch-emerald/20">
        ...
        <div className="p-2 bg-ve-red/10 border border-ve-red/20">
          <span className="text-[9px] uppercase text-ve-red block font-bold">Misses</span>
          <strong className="text-ve-red font-bold text-sm">{performance.misses}</strong>
        </div>
      </div>
    </article>
  );
...
export default function BrainPerformancePage({ onNavigate }: { onNavigate: (section: string) => void }) {
  ...
  return (
    <BrainPageShell active="performance" onNavigate={onNavigate}>
      {/* Top Banner */}
      <section className="brain-panel brain-callout p-5 sm:p-6 border border-vouch-cyan/30 bg-vouch-cyan/5">
        <div className="flex gap-4">
          <span className="grid h-10 w-10 shrink-0 place-items-center border border-vouch-emerald/30 bg-vouch-emerald/10 text-vouch-emerald">
            <ShieldCheck className="h-5 w-5" />
          </span>
          <div>
            <div className={`${AURORA_LABEL} text-vouch-emerald font-bold`}>Immutable Win / Loss Accountability Ledger</div>
            <h2 className="mt-1 text-xl font-bold text-white">Truth-First Database Performance Record</h2>
            <p className="mt-2 max-w-3xl text-xs sm:text-sm leading-relaxed text-white/70">
      ...
      {query.isLoading && (
        <section className="brain-panel flex min-h-48 items-center justify-center gap-3 p-6 border border-white/10 bg-black/40">
          <RefreshCw className="h-5 w-5 animate-spin text-vouch-cyan" />
          <span className={`${AURORA_LABEL} text-white/70`}>Querying database performance ledger...</span>
        </section>
      )}

      {query.isError && (
        <section className="brain-panel p-8 text-center border border-ve-red/20 bg-ve-red/5">
          <Activity className="mx-auto h-8 w-8 text-ve-red" />
      ...
      {!query.isLoading && !query.isError && !hasData && (
        <div className="space-y-6">
          <section className="brain-panel p-8 text-center border border-white/10 bg-black/40 space-y-3">
            <Clock className="mx-auto h-10 w-10 text-vouch-cyan animate-pulse" />
            <h2 className="text-xl font-bold text-white">Awaiting Official Game Box Scores</h2>
            <p className="text-sm text-white/70 max-w-lg mx-auto leading-relaxed">
              All Brain decisions for today's slate are currently <strong className="text-ve-amber font-mono font-bold uppercase">PENDING</strong>. 
      ...
      {hasData && ledger && (
        <div className="space-y-6">
          {/* Main Record Header */}
          <section className="brain-panel p-5 sm:p-6 border border-white/10 bg-black/40">
            <div className="flex flex-col gap-4 border-b border-white/10 pb-5 sm:flex-row sm:items-end sm:justify-between">
              <div>
                <div className={`${AURORA_LABEL} text-vouch-cyan`}>Verified Home Run Record</div>
                <h2 className="mt-1 text-2xl font-bold text-white tracking-tight">Model Outcomes & Win Rate</h2>
              </div>
              <div className="text-left sm:text-right">
                <strong className="font-mono text-4xl font-bold text-vouch-emerald">
                  {ledger.performance.hitRate == null ? '—' : `${ledger.performance.hitRate}%`}
                </strong>
                <span className={`${AURORA_LABEL} block text-white/55`}>Verified Hit Rate</span>
              </div>
            </div>
            ...
            <div className="brain-stat-grid mt-5">
              {[
                ...
                <div key={String(label)} className="brain-stat p-3 bg-white/[0.02] border border-white/5">
                  <span className={`${AURORA_LABEL} text-white/55`}>{label}</span>
                  <strong className="mt-1 block font-mono text-2xl font-bold text-white">{value}</strong>
                </div>
              ))}
            </div>
          </section>
          ...
          <section className="grid gap-4 xl:grid-cols-[minmax(0,1.2fr)_minmax(18rem,0.8fr)]">
            <article className="brain-panel p-5 sm:p-6 border border-white/10 bg-black/40">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between border-b border-white/10 pb-4">
                <div>
                  ...
                  <h2 className="mt-1 text-lg font-bold text-white uppercase tracking-wider">Frozen Wins & Losses by Date</h2>
                </div>
                ...
                {ledger.picks.map((pick) => {
                  ...
                  return (
                    <div
                      ...
                      className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border border-white/10 bg-white/[0.02] p-3.5 hover:border-white/20 transition"
                    >
                      <div className="min-w-0 space-y-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          {/* Dedicated Prominent Date Pill */}
                          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 border border-white/15 bg-white/5 font-mono text-xs font-bold text-vouch-cyan">
                        ...
                        <span className="block font-mono text-[11px] uppercase text-white/55">
                          {pick.team} vs {pick.opponent} · Model Score {pick.score} · {pick.evidenceQuality}
                        </span>
                      </div>

                      <div className="shrink-0 flex items-center gap-1.5 font-mono text-xs font-semibold uppercase self-start sm:self-center">
                        {isHit && (
                        ...
                        {isMiss && (
                          <span className="flex items-center gap-1 text-ve-red border border-ve-red/40 bg-ve-red/10 px-2.5 py-1 rounded-full">
                            <XCircle className="h-3.5 w-3.5" /> LOSS (MISS)
                          </span>
                        )}
                        {isPending && (
                          <span className="flex items-center gap-1 text-ve-amber border border-ve-amber/40 bg-ve-amber/10 px-2.5 py-1 rounded-full">
                            <Clock className="h-3.5 w-3.5" /> PENDING
                          </span>
                        )}
                        {isVoid && (
                          <span className="flex items-center gap-1 text-white/55 border border-white/40 bg-white/40 px-2.5 py-1 rounded-full">
            ...
            {/* Protocol Card */}
            <aside className="brain-panel p-5 sm:p-6 border border-white/10 bg-black/40 space-y-4">
              ...
              <div className="space-y-3 text-xs text-white/70 leading-relaxed font-mono">
                {[
                  ...
                  <div key={item} className="flex gap-2.5 p-2.5 border border-white/5 bg-white/[0.02]">
