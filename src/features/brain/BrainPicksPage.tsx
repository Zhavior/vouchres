        ...
        <div className="flex flex-col gap-3 border-b border-white/10 pb-4 sm:flex-row sm:items-end sm:justify-between">
          ...
          {scanQuery.data && (
            <div className="flex flex-wrap gap-2">
              <span
                className={`${AURORA_LABEL} border border-vouch-cyan/25 bg-vouch-cyan/5 px-3 py-1.5 text-vouch-cyan`}
              >
                {scanQuery.data.scan.coverage.playersScanned} players ·{" "}
                {scanQuery.data.scan.coverage.games} games
              </span>
              <span
                className={`${AURORA_LABEL} border px-3 py-1.5 ${scanQuery.data.scan.temporal.decisionWindowOpen ? "border-vouch-emerald/30 bg-vouch-emerald/8 text-vouch-emerald" : "border-white/10 text-white/45"}`}
              >
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    ...
                    <span
                      className={`${AURORA_LABEL} border border-vouch-cyan/25 bg-vouch-cyan/5 px-2 py-1 text-vouch-cyan`}
                    >
                    ...
                    {(review?.tags ?? pick.reasons ?? [])
                      ...
                      .map((reason) => (
                        <span
                          key={reason}
                          className={`${AURORA_LABEL} border border-white/10 px-2 py-1 text-white/50`}
                        >
                          {reason}
                        </span>
                      ))}
            ...
            <article className="brain-panel min-w-0 p-4 sm:p-6 space-y-4">
              <div className="flex flex-col gap-4 border-b border-white/10 pb-5 sm:flex-row sm:items-start sm:justify-between">
                ...
                <div className="border border-vouch-emerald/25 bg-vouch-emerald/8 px-4 py-3 text-center rounded-xl">
                  ...
                  <span className={`${AURORA_LABEL} block text-white/40`}>
                    Slate percentile
                  </span>
                </div>
              </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    ...
                    <span
                      className={`${AURORA_LABEL} border border-vouch-cyan/25 px-2 py-1 text-vouch-cyan`}
                    >
                      AI: {reviewLabel(review)}
                    </span>
