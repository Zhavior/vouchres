...
type LandingVouchCursorTipProps = {
  title: string;
  body: string;
  children: ReactNode;
};
...
export default function LandingVouchCursorTip({ title, body, children }: LandingVouchCursorTipProps) {
  ...
  return (
    <>
      <div
        ref={anchorRef}
        className="ve-landing-vouch-tip-anchor"
        onPointerEnter={show}
        onPointerMove={show}
        onPointerLeave={hide}
      ...
      {active && typeof document !== 'undefined'
        ? createPortal(
            <div
              className="ve-landing-vouch-cursor-tip pointer-events-none fixed z-[120] max-w-[16rem] rounded-lg border border-white/15 bg-[#0c111b]/95 px-3 py-2.5 shadow-[0_12px_40px_rgba(0,0,0,0.55)] backdrop-blur-md"
              style={tipStyle}
              role="tooltip"
            >
