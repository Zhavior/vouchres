  ...
  return (
    <div className="flex items-center gap-2.5">
      ...
      <div>
        ...
        <p className="font-mono text-[7px] font-bold uppercase tracking-[0.25em] text-[#a8d8b6]/65">Daily desk</p>
      </div>
    </div>
  );
}
...
function UtilityButton({ children, active, onClick }: { children: React.ReactNode; active?: boolean; onClick: () => void }) {
  return (
    <button type="button" onClick={onClick} aria-pressed={active} className={`inline-flex min-h-9 items-center justify-center gap-1.5 border px-3 text-[9px] font-semibold transition ${active ? 'border-[#8bcda0]/35 bg-[#8bcda0]/10 text-[#c9ead3]' : 'border-white/10 bg-white/[0.025] text-white/55 hover:border-white/20 hover:text-white'}`}>
      {children}
    </button>
  );
}
...
function Spotlight({ game, compact, saved, onToggleSaved }: { game: DemoGame; compact: boolean; saved: boolean; onToggleSaved: () => void }) {
  return (
    <article className="overflow-hidden border border-white/[0.1] bg-[#07100c]/95 shadow-[0_36px_90px_-42px_rgba(0,0,0,0.95)]">
      <div className="flex min-h-8 items-center justify-between border-b border-white/[0.07] bg-[#0b1510] px-4">
        <span className="flex items-center gap-2 font-mono text-[8px] font-bold uppercase tracking-[0.16em] text-[#8bcda0]"><Radio className="h-3 w-3" /> Spotlight</span>
        <span className="flex items-center gap-1.5 text-[8px] text-white/40"><Clock3 className="h-3 w-3" /> Updated {game.receipt.updated}</span>
      </div>

      <div className={`grid ${compact ? '' : 'lg:grid-cols-[0.84fr_1.16fr]'}`}>
        <div className={`p-4 ${compact ? 'border-b border-white/[0.07]' : 'border-b border-white/[0.07] lg:border-b-0 lg:border-r'}`}>
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="font-mono text-[8px] font-bold uppercase tracking-[0.17em] text-white/35">{game.away} @ {game.home} · {game.time}</p>
              <h2 className="mt-1 text-lg font-semibold tracking-[-0.03em] text-[#f2f0e9]">{game.player}</h2>
              <p className="mt-1 text-[9px] text-white/40">{game.team} · {game.status}</p>
            </div>
            <div className="border border-[#8bcda0]/20 bg-[#8bcda0]/[0.06] px-3 py-2 text-right">
              <p className="font-mono text-2xl tabular-nums text-[#f2f0e9]">{game.score}</p>
              <p className="font-mono text-[7px] uppercase tracking-[0.13em] text-[#8bcda0]">HRPI / 100</p>
            </div>
          </div>

          <p className="mt-3 text-[11px] leading-5 text-[#c7cbc3]">{game.read}</p>
          <div className="mt-3 flex items-center justify-between gap-3 border-t border-white/[0.07] pt-3">
            <div>
              <p className="text-[10px] font-semibold text-[#dfe2da]">{game.signal}</p>
              <p className="mt-1 text-[8px] text-white/35">{game.evidenceConfidence} · score is not a guarantee.</p>
            </div>
        ...
        <div className="p-4">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-[10px] text-white/50">Why this row ranks here.</p>
            </div>
            <span className="border border-[#8bcda0]/20 bg-[#8bcda0]/[0.07] px-2 py-1 font-mono text-[7px] font-bold uppercase tracking-[0.12em] text-[#a8d8b6]">{game.evidenceConfidence}</span>
          </div>
          <div className={`mt-3 grid gap-2 ${compact ? '' : 'sm:grid-cols-2'}`}>
            {game.evidence.map((item) => (
              <div key={item.label} className="border border-white/[0.07] bg-white/[0.025] p-2.5">
                <div className="flex items-start justify-between gap-2">
                  <span className="text-[9px] text-white/45">{item.label}</span>
                  <span className={`border px-1.5 py-0.5 font-mono text-[7px] font-bold uppercase ${TONE_STYLE[item.tone]}`}>{item.value}</span>
                </div>
                <div className="mt-2 flex gap-1">
                  {[0, 1, 2, 3, 4].map((bar) => <span key={bar} className={`h-1 flex-1 ${bar < (item.tone === 'good' ? 4 : item.tone === 'neutral' ? 3 : 2) ? item.tone === 'watch' ? 'bg-[#e5ad62]/70' : 'bg-[#8bcda0]/70' : 'bg-white/[0.06]'}`} />)}
                </div>
              </div>
            ))}
          </div>
  ...
  return (
    <div className="border-x border-b border-[#8bcda0]/20 bg-[#0a1510] px-4 py-4" role="region" aria-label={`${game.away} at ${game.home} research receipt`}>
      <div className="flex items-start justify-between gap-4">
        <div className="flex gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-[8px] text-white/35">Captured {game.receipt.updated} · original conclusion preserved</p>
          </div>
        </div>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-3">
        <div><p className="font-mono text-[7px] font-bold uppercase tracking-[0.15em] text-white/30">Sources</p><p className="mt-1.5 text-[9px] leading-4 text-white/60">{game.receipt.sources.join(' · ')}</p></div>
        <div><p className="font-mono text-[7px] font-bold uppercase tracking-[0.15em] text-white/30">Missing inputs</p><p className="mt-1.5 text-[9px] leading-4 text-white/60">{game.receipt.missing}</p></div>
        <div><p className="font-mono text-[7px] font-bold uppercase tracking-[0.15em] text-white/30">Method</p><p className="mt-1.5 text-[9px] leading-4 text-white/60">{game.receipt.methodology}</p></div>
      </div>
    </div>
  );
}
...
function SlateQueue({ games, activeId, compact, savedIds, receiptId, onSelect, onToggleSaved, onToggleReceipt }: {
  ...
  return (
    <div className="border border-white/[0.08] bg-[#070c09]">
      <div className={`hidden min-h-9 items-center border-b border-white/[0.07] bg-white/[0.02] px-3 font-mono text-[7px] font-bold uppercase tracking-[0.13em] text-white/25 ${compact ? '' : 'sm:grid sm:grid-cols-[1.1fr_1fr_0.55fr_0.7fr_0.65fr_70px]'}`}>
      ...
      {games.length === 0 ? <p className="px-4 py-10 text-center text-[10px] text-white/40">No matchups match the active workspace filters.</p> : null}
      {games.map((game) => {
        const active = game.id === activeId;
        const receiptOpen = receiptId === game.id;
        return (
          <div key={game.id}>
            <div className={`grid border-b border-white/[0.06] transition ${compact ? 'grid-cols-[1fr_auto]' : 'sm:grid-cols-[1fr_70px]'} ${active ? 'bg-[#8bcda0]/[0.08]' : 'hover:bg-white/[0.025]'}`}>
              <button type="button" onClick={() => onSelect(game.id)} aria-pressed={active} className={`grid min-w-0 gap-3 px-3 py-3 text-left ${compact ? 'grid-cols-2' : 'sm:grid-cols-[1.1fr_1fr_0.55fr_0.7fr_0.65fr] sm:items-center'}`}>
                <span>
                  <span className="block font-mono text-[10px] font-bold text-[#e9e8e1]">{game.away} @ {game.home}</span>
                  <span className="mt-1 block text-[8px] text-white/35">{game.time}</span>
                </span>
                <span className={compact ? 'text-right' : ''}>
                  <span className="block text-[10px] font-semibold text-white/70">{game.player}</span>
                  <span className="mt-1 block text-[8px] text-white/30">{game.team}</span>
                </span>
                <span className={`font-mono text-sm font-semibold tabular-nums ${active ? 'text-[#b9e8c8]' : 'text-white/70'}`}>{game.score}</span>
                <span className={`inline-flex w-fit items-center gap-1 border px-1.5 py-1 font-mono text-[7px] font-bold uppercase ${game.confirmed ? TONE_STYLE.good : TONE_STYLE.watch}`}><CircleDot className="h-2.5 w-2.5" /> {game.confirmed ? 'Confirmed' : 'Projected'}</span>
                <span><span className="font-mono text-[9px] text-white/55">{game.marketVolume}</span><span className="ml-1 text-[7px] text-white/25">index</span></span>
              </button>
              <span className={`flex items-center justify-end gap-2 px-3 ${compact ? 'flex-col border-l border-white/[0.06]' : ''}`}>
                <button type="button" onClick={() => onToggleSaved(game.id)} aria-label={`${savedIds.has(game.id) ? 'Remove' : 'Add'} ${game.player} ${savedIds.has(game.id) ? 'from' : 'to'} My List`} className={`grid h-7 w-7 place-items-center border ${savedIds.has(game.id) ? 'border-[#8bcda0]/30 text-[#8bcda0]' : 'border-white/10 text-white/25 hover:text-white'}`}><Star className={`h-3 w-3 ${savedIds.has(game.id) ? 'fill-current' : ''}`} /></button>
                <button type="button" onClick={() => onToggleReceipt(game.id)} aria-label={`${receiptOpen ? 'Close' : 'Open'} ${game.player} receipt`} className={`grid h-7 w-7 place-items-center border ${receiptOpen ? 'border-[#8bcda0]/30 bg-[#8bcda0]/10 text-[#8bcda0]' : 'border-white/10 text-white/30 hover:text-white'}`}><ChevronDown className={`h-3 w-3 transition ${receiptOpen ? 'rotate-180' : ''}`} /></button>
              </span>
            </div>
...
function DailyHubConcept({ compact }: { compact: boolean }) {
  ...
  return (
    <div className="min-h-full bg-[#060907] text-[#f2f0e9]">
      <header className="border-b border-white/[0.07] bg-[#080d0a]">
        <div className="flex min-h-16 items-center justify-between gap-3 px-4 sm:px-6">
          <ProductMark />
          <div className="flex items-center gap-2">
            <span className="hidden items-center gap-1.5 font-mono text-[8px] uppercase tracking-[0.12em] text-[#8bcda0] sm:flex"><CircleDot className="h-2.5 w-2.5" /> Sources fresh</span>
      ...
      <main className="mx-auto max-w-[1240px] px-3 py-4 sm:px-6 sm:py-6">
        <div className={`flex gap-4 ${compact ? 'flex-col' : 'flex-col lg:flex-row lg:items-end lg:justify-between'}`}>
          <div>
            <div className="flex items-center gap-2 font-mono text-[8px] font-bold uppercase tracking-[0.18em] text-[#8bcda0]"><Radio className="h-3 w-3" /> Tuesday · Daily research session</div>
            <h1 className="mt-2 text-2xl font-semibold tracking-[-0.04em] text-[#f2f0e9] sm:text-3xl">Today&apos;s MLB research desk</h1>
            <p className="mt-2 text-[10px] text-white/40">6 sample matchups · 4 confirmed lineups · refreshed 7:44 PM</p>
          </div>
          <div className={`flex gap-2 ${compact ? 'grid grid-cols-2 [&>button:last-child]:col-span-2' : 'flex-wrap'}`}>
            <UtilityButton active={confirmedOnly} onClick={() => setConfirmedOnly((value) => !value)}><Filter className="h-3.5 w-3.5" /> {confirmedOnly ? 'Confirmed only' : 'Filter slate'}</UtilityButton>
        ...
        {exportStatus ? <div className="mt-3 flex items-center gap-2 border border-[#8bcda0]/20 bg-[#8bcda0]/[0.07] px-3 py-2 text-[9px] text-[#c9ead3]" role="status"><Check className="h-3.5 w-3.5" /> {exportStatus}</div> : null}

        <div className="mt-4"><Spotlight game={game} compact={compact} saved={savedIds.has(game.id)} onToggleSaved={() => toggleSaved(game.id)} /></div>

        <section className="mt-3" aria-labelledby="slate-queue-title">
          <div className="mb-2.5 flex items-end justify-between gap-4">
            <div>
              <p className="font-mono text-[8px] font-bold uppercase tracking-[0.17em] text-[#8bcda0]">Daily queue</p>
              <h2 id="slate-queue-title" className="mt-1 text-base font-semibold text-[#e9e8e1]">Remaining slate</h2>
            </div>
            <p className="text-right text-[8px] leading-4 text-white/30">Sorted by {SORT_LABELS[sortKey]}<br />Market attention is sample prototype data.</p>
          </div>
        ...
        <div className="mt-4 grid gap-2 border border-white/[0.07] bg-[#0a110d] p-3 sm:grid-cols-3">
          ...
          ].map(({ icon: Icon, text }) => <div key={text} className="flex items-center gap-2 text-[8px] text-white/35"><Icon className="h-3.5 w-3.5 text-[#8bcda0]" /> {text}</div>)}
        </div>
      </main>
    </div>
  );
  ...
  return (
    <section className="space-y-4" aria-labelledby="front-page-lab-title">
      <div className="flex flex-col gap-4 rounded-lg border border-white/10 bg-black/20 p-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div className="flex items-center gap-2 font-mono text-[9px] font-bold uppercase tracking-[0.17em] text-vouch-emerald"><Sparkles className="h-3.5 w-3.5" /> Isolated concept preview</div>
          <h2 id="front-page-lab-title" className="mt-1.5 text-lg font-semibold text-white">Today Hub Concept 02 — Daily Research Desk</h2>
          <p className="mt-1 max-w-2xl text-xs leading-5 text-white/45">Sample content for interaction and visual review only. This does not replace or modify the current Today page.</p>
        </div>
      ...
      <div className="flex items-start gap-3 rounded-lg border border-vouch-emerald/15 bg-vouch-emerald/[0.05] p-4">
        ...
        <p className="text-xs leading-5 text-white/55"><strong className="text-white/80">Review target:</strong> judge this as an authenticated daily workbench—not a marketing page. Rows update the spotlight and expand receipts instantly without navigation or reloads.</p>
      </div>
    </section>
  );
}
