...
export default function ThemeStore({
  ...
  const [activeTab, setActiveTab] = useState<"locker" | "shop" | "custom">(
    "locker",
  );
  ...
  const [previewBorderId, setPreviewBorderId] = useState<string>(
    currentBorder?.id || "default-cyber-ring",
  );
  ...
  const [mGlowGradient, setMGlowGradient] = useState(
  ...
  const handleClaimCredits = () => {
    ...
    triggerSuccess(
      "🎉 Claimed +500 FREE theme creation credits from VouchEdge Creator Pool!",
    );
  };
  ...
  const handleBuyTheme = (theme: VisualTheme) => {
    if (userCredits < theme.cost) {
      alert(
        "❌ Insufficient store credits! Please click 'Claim Free Credits' to top up your balance instantly.",
      );
      return;
    }
    ...
    triggerSuccess(
      `💎 Successfully purchased "${theme.name}"! You can now equip it as your App or Profile theme in your Locker tab.`,
    );
  };
  ...
  const handleBuyBorder = (border: ProfileBorder) => {
    ...
    triggerSuccess(
      `🛡️ Successfully purchased "${border.name}" avatar frame! Equip it instantly in your Locker tab.`,
    );
  };
  ...
  const handleGenerateAITheme = async (e: React.FormEvent) => {
    ...
    try {
      const data = await apiClient.post<{
        ...
        theme?: VisualTheme;
        error?: string;
      }>("/api/ai/generate-theme", { prompt: aiPrompt.trim() });
      ...
      if (data.status === "success" || data.status === "simulated") {
        const generatedTheme: VisualTheme = {
          ...
          cardStyle:
            "bg-ve-storm/90 border-cyan-400/30 shadow-[0_0_15px_rgba(34,211,238,0.15)]",
        ...
        localStorage.setItem(
          accountStorageKey("vouchedge_market_themes"),
          JSON.stringify(updatedMarketList),
        );
        ...
        setAiPrompt("");
        triggerSuccess(
          `🔮 Beautiful! Gemini 3.5 AI has synthesized, coded, and unlocked "${generatedTheme.name}" inside your locker!`,
        );
      } else {
        setAiError(
          data.error ||
            "The Gemini AI engine had trouble synthesizing this visual styling combination. Please try an alternate prompt!",
        );
      }
    } catch (err: any) {
      setAiError(
        err.message ||
          "Failure reaching Google AI Studio theme generation cluster.",
      );
    } finally {
  ...
  const handleMintTheme = (e: React.FormEvent) => {
    ...
    const cleanParticles = mParticleDemo
      .split(",")
      .map((p) => p.trim())
      .filter(Boolean);
    ...
    const newTheme: VisualTheme = {
      ...
      backgroundPattern: "grid",
      cardStyle:
        "bg-ve-graphite border-cyan-500/30 shadow-[0_0_15px_rgba(6,182,212,0.15)] backdrop-blur-md",
    ...
    triggerSuccess(
      `🚀 Custom theme "${newTheme.name}" successfully minted & added to locker! Payouts listed to community.`,
    );
  };
  ...
  const handleResetToDefault = () => {
    ...
    triggerSuccess(
      "🌿 Profile and App themes reset to baseline Cyber Blue standard.",
    );
  };

  const selectedPreviewTheme =
    allThemes.find((t) => t.id === previewThemeId) || allThemes[0];
  const selectedPreviewBorder =
    BORDER_REGISTRY.find((b) => b.id === previewBorderId) || BORDER_REGISTRY[0];
  ...
  return (
    <div
      className="p-4 md:p-6 space-y-6 max-w-[1200px] mx-auto min-h-screen bg-transparent"
      id="theme-store-root"
    >
      {/* Visual Header Banners */}
      <div
        className="bg-ve-storm/30 backdrop-blur-md rounded-2xl border border-white/[0.06] p-5 md:p-7 flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden shadow-2xl"
        id="theme-banner"
      >
        ...
        <div className="space-y-2 text-left z-10">
          ...
          <p className="text-xs text-white/45 max-w-2xl leading-relaxed">
            Customize your client application interface and configure your
            public profile identity. Your public profile page temporarily
            transforms to your selected theme for visiting followers, complete
            with custom avatar borders, badges, cards, and animations!
          </p>
        </div>

        {/* User Balance Desk */}
        <div
          className="bg-black/30 border border-white/[0.08] p-4 rounded-xl flex flex-col items-center gap-2 text-center shrink-0 min-w-[200px] shadow-inner"
          id="user-balance-desk"
        >
          <span className="text-[9px] text-white/40 font-bold uppercase tracking-wider font-mono">
          ...
          <div className="lg:col-span-8 space-y-6">
              ...
              <div className="flex items-center justify-between bg-obsidian-900/60 p-4 rounded-xl border border-white/10">
                <div className="space-y-1 pr-4">
                  ...
                  <p className="text-[11px] text-white/45">
                    Disable looping background particles, spinning rings, bounce
                    effects, and custom text waves to optimize CPU & legibility.
                  </p>
                </div>
                <button
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {allThemes
                  .filter((t) => unlockedThemes.includes(t.id))
                  .map((theme) => {
                      ...
                      >
                        ...
                        <div className="relative z-10 space-y-4">
                          <div className="flex justify-between items-center gap-3">
                            ...
                            <span
                              className={`text-[9px] font-black font-mono px-2.5 py-1 rounded-full border ${
                                theme.rarity === "legendary"
                                  ? "text-yellow-200 bg-yellow-400/15 border-yellow-300/35"
                                  : theme.rarity === "epic"
                                    ? "text-purple-200 bg-purple-400/15 border-purple-300/35"
                          ...
                          <div>
                            <h4 className="font-black text-white/90 text-sm flex flex-wrap items-center gap-1.5">
                              {theme.name}
                              {isAppTheme && (
                                <span className="text-[8px] bg-sky-400/15 border border-sky-300/25 text-sky-200 px-2 py-0.5 rounded-full font-black font-mono">
                                  CURRENT APP
                                </span>
                              )}
                              {isProfileTheme && (
                                <span className="text-[8px] bg-purple-400/15 border border-purple-300/25 text-purple-200 px-2 py-0.5 rounded-full font-black font-mono">
                                  CURRENT PROFILE
                                </span>
                              )}
                            </h4>
                        ...
                        >
                          <button
                            onClick={() => {
                              ...
                              triggerSuccess(
                                `✨ "${theme.name}" equipped globally as your personal App Theme!`,
                              );
                            }}
                            disabled={isAppTheme}
                          ...
                          <button
                            onClick={() => {
                              ...
                              triggerSuccess(
                                `🔮 "${theme.name}" equipped publicly as your Profile Theme!`,
                              );
                            }}
                            disabled={isProfileTheme || !canEditProfileHeader}
                            title={
                              !canEditProfileHeader
                                ? "Upgrade to Gold to customize your profile header"
                                : undefined
                            }
                            className={`py-2 text-[10px] font-black uppercase tracking-wider rounded-xl border text-center transition-all ${
                              !canEditProfileHeader
            ...
            <div className="space-y-3 pt-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {BORDER_REGISTRY.filter((b) =>
                  unlockedBorders.includes(b.id),
                ).map((border) => {
                    ...
                    >
                      ...
                      <button
                        onClick={(e) => {
                          ...
                          setBorder(border.id);
                          triggerSuccess(
                            `🛡️ Profile picture frame changed successfully to "${border.name}"!`,
                          );
                        }}
                        disabled={isActive}
          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="group relative overflow-hidden bg-ve-storm/45 backdrop-blur-xl rounded-3xl border border-white/[0.08] p-5 space-y-5 shadow-2xl sticky top-4">
              ...
              <div className="relative z-10 border-b border-white/10 pb-4">
                ...
                <p className="text-[10px] text-white/45 font-semibold mt-1">
                  See how your cards, borders, and typography appear.
                </p>
              </div>

              {/* LIVE DEMO BLOCK COMPILING SELECTS */}
              <div
                className={`relative z-10 p-4 rounded-3xl border ${selectedPreviewTheme.cardStyle} space-y-4 shadow-xl`}
                id="preview-sandbox-card"
              >
                {/* Header Row */}
                <div className="flex justify-between items-center text-[10px] uppercase font-bold text-white/45 font-mono">
                  ...
                  <span
                    className={`${selectedPreviewTheme.accentText} bg-obsidian-900/45 px-2 py-0.5 rounded border ${selectedPreviewTheme.borderColor}`}
                  >
                ...
                <div className="flex items-center gap-3.5 border-b border-white/[0.04] pb-3">
                  ...
                  <div className="min-w-0">
                    <h4 className="font-extrabold text-white/90 text-sm">
                      {profile.displayName}
                    </h4>
                ...
                <div
                  className={`p-3.5 rounded-xl border ${selectedPreviewTheme.vouchCardStyle} text-xs space-y-2.5`}
                >
                  ...
                  <div>
                    ...
                    <p className="text-[10.5px] text-white/45 leading-normal mt-1">
                      Ohtani matches perfectly under Dodgers stadium lights
                      tonight with massive wind blowing outward.
                    </p>
                  </div>
                  <div className="flex justify-between items-center font-mono text-[10px] border-t border-white/[0.04] pt-2">
                    ...
                    <span
                      className={`font-black ${selectedPreviewTheme.accentText}`}
                    >
                      Odds: -115
                    </span>
                  </div>
                ...
                {/* Theme Action Button */}
                <button
                  className={`w-full py-2.5 rounded-xl text-center text-xs font-black uppercase ${selectedPreviewTheme.buttonStyle}`}
                >
              ...
              <div className="relative z-10 bg-black/30 p-3.5 rounded-2xl border border-white/10 text-xs text-white/45 space-y-2.5 shadow-inner">
                <span className="text-[9.5px] font-mono font-black text-vouch-cyan uppercase tracking-wider block">
                  PREVIEW DETAILS:
                </span>
                  ...
                  <div className="flex justify-between text-white/40">
                    ...
                    <span
                      className={`capitalize font-bold ${
                        selectedPreviewTheme.rarity === "legendary"
                          ? "text-yellow-400"
                          : selectedPreviewTheme.rarity === "epic"
                            ? "text-purple-400"
                            : "text-white/45"
                      }`}
                    >
                      {selectedPreviewTheme.rarity}
                    </span>
                  </div>
                </div>
              </div>
        ...
        <div className="space-y-8">
          ...
          <div className="space-y-4 text-left">
            <h3 className="text-xs font-bold text-white/65 uppercase tracking-widest flex items-center gap-1.5">
              ...
              Locked Theme Catalog (
              {
                allThemes.filter((t) => !unlockedThemes.includes(t.id)).length
              }{" "}
              Available)
            </h3>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="relative z-10 p-4 bg-obsidian-900/55 border-b border-white/10 flex justify-between items-center text-[10px] uppercase font-bold text-white/65 font-mono select-none">
                      <span className="rounded-full bg-black/30 border border-white/10 px-2.5 py-1">
                        {theme.category}
                      </span>
                      <span
                        className={`font-black px-2.5 py-1 rounded-full border ${
                          theme.rarity === "legendary"
                            ? "text-yellow-200 bg-yellow-400/15 border-yellow-300/35"
                            : theme.rarity === "epic"
                              ? "text-purple-200 bg-purple-400/15 border-purple-300/35"
          ...
          <div className="space-y-4 pt-6 text-left border-t border-slate-850">
            <h3 className="text-xs font-bold text-white/65 uppercase tracking-widest flex items-center gap-1.5">
              <Shield className="w-4 h-4 text-emerald-400" />
              Locked Avatar Frames (
              {
                BORDER_REGISTRY.filter((b) => !unlockedBorders.includes(b.id))
                  .length
              }{" "}
              Available)
            </h3>
                    ...
                    <div className="flex gap-3 items-center min-w-0">
                      ...
                      <div className="min-w-0">
                        <h4 className="font-extrabold text-white/80 text-xs truncate flex items-center gap-1">
                          {border.name}
                          <span
                            className={`text-[8px] font-black font-mono px-1 py-0.2 rounded uppercase ${
                              border.rarity === "legendary"
        ...
        <div className="space-y-6">
          ...
          <div className="bg-ve-storm/30 backdrop-blur-md rounded-2xl border border-white/10 p-6 text-left space-y-4">
            ...
            <p className="text-xs text-white/45 max-w-3xl leading-relaxed">
              Connect to our integrated Gemini capper models to synthesize,
              code, and list a brand-new, bespoke visual theme dynamically.
              Provide visual mood cues (e.g. "cyberpunk ballpark under storm",
              "liquid neon gold starfighter"), and watch the AI write the layout
              classes instantly!
            </p>
            ...
            <form
              onSubmit={handleGenerateAITheme}
          ...
          <div
            className="bg-ve-storm/35 backdrop-blur-md rounded-2xl border border-dashed border-white/10 p-6 md:p-8 space-y-6 relative overflow-hidden"
            id="creator-licensing-launchpad"
          >
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/10 pb-5">
              <div className="space-y-1 text-left">
                ...
                <p className="text-xs text-white/45 max-w-2xl leading-relaxed">
                  Generate custom cosmetic theme templates. Mint them directly
                  onto the Vouch-ledger to sell to your fans and copy-bettors!
                  Set your own price index and earn an instant{" "}
                  <span className="text-emerald-400 font-extrabold">
                    85% custom royalty share
                  </span>{" "}
                  on every user download.
                </p>
              </div>
              ...
              <div className="bg-obsidian-900/80 p-3.5 rounded-xl border border-slate-850 grid grid-cols-2 gap-4 font-mono text-center min-w-[240px] shadow-inner">
                <div>
                  <span className="text-[8px] text-white/40 uppercase block font-bold">
                    Total Sales
                  </span>
                ...
                <div>
                  ...
                  <span className="text-xs text-vouch-cyan font-extrabold">
                    85% Creator
                  </span>
                </div>
              </div>
            ...
            <form
              onSubmit={handleMintTheme}
              className="grid grid-cols-1 lg:grid-cols-12 gap-6 text-left"
              id="mint-custom-theme-form"
            >
              {/* Left panel instructions/inputs */}
              <div className="lg:col-span-7 space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="text-[10px] uppercase font-mono tracking-wider font-extrabold text-white/45">
                      Theme Display Name
                    </label>
                    <input
                      type="text"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    ...
                    >
                      <option value="border-dashed border-rose-500">
                        Dashed Cute Pink (Cat-like)
                      </option>
                      <option value="border-emerald-500 border-2">
                        Sleek Green Matrix
                      </option>
                      <option value="border-blue-500 border-4">
                        Thick Google Blue
                      </option>
                      <option value="border-yellow-400">
                        Amber Glow Crown
                      </option>
                      <option value="border-purple-600 animate-spin">
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[10px] uppercase font-mono tracking-wider font-extrabold text-white/45">
                      Demo Floating Particles (Comma Sep)
                    </label>
                    <input
                      type="text"
                      placeholder="e.g. 🐱, 🐾, 💎, 🐈"
              ...
              <div
                className="lg:col-span-5 bg-black/25 p-5 rounded-2xl border border-slate-850/80 flex flex-col justify-between gap-6"
                id="minting-live-calculator-panel"
              >
                <div className="space-y-4">
                  ...
                  <div className="space-y-2 font-mono text-xs">
                    ...
                    <div className="p-3 bg-obsidian-900/60 rounded-xl border border-white/10 space-y-1.5 text-[10.5px] text-white/45 leading-normal">
                      <span className="text-[#FBBC05] font-black block uppercase text-[9.5px]">
                        💡 SELLER INSTRUCTIONS:
                      </span>
                      <p>
                        When user mints a layout, VouchEdge locks your
                        configuration parameters in your web context. Other
                        Bettors inspect your theme and can instantly unlock it
                        via the store. Your credit balance will update
                        dynamically!
                      </p>
                    </div>
                  </div>
