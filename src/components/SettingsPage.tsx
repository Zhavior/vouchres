...
function normalizeTier(tier?: string | null): AppTier {
  ...
  if (t === "SELLER_PRO" || t === "SELLER PRO" || t === "PRO")
    return "SELLER_PRO";
  return "BASIC";
}
...
export default function SettingsPage({
  ...
  const [emailAlerts, setEmailAlerts] = useState(
    () =>
      localStorage.getItem(accountStorageKey("vouchedge_email_alerts")) !==
      "false",
  );
  const [pushAlerts, setPushAlerts] = useState(
    () =>
      localStorage.getItem(accountStorageKey("vouchedge_push_alerts")) !==
      "false",
  );
  ...
  const [weeklySummary, setWeeklySummary] = useState(
    () =>
      localStorage.getItem(accountStorageKey("vouchedge_weekly_summary")) !==
      "false",
  );
  const [profilePublic, setProfilePublic] = useState(
    () =>
      localStorage.getItem(accountStorageKey("vouchedge_profile_public")) !==
      "false",
  );
  const [reduceMotion, setReduceMotion] = useState(
    Boolean(profile.reduceMotion),
  );
  ...
  useEffect(() => {
    ...
    localStorage.setItem(
      accountStorageKey("vouchedge_push_alerts"),
      String(pushAlerts),
    );
    localStorage.setItem(
      accountStorageKey("vouchedge_weekly_summary"),
  ...
  const refreshBilling = async (message?: string) => {
    ...
    if (!status) {
      showToast(
        "Billing status unavailable — sign in and configure Stripe.",
        "err",
      );
      return;
    }
  ...
  const handleUpgrade = async (tier: AppTier) => {
    ...
    const checkoutError =
      "error" in result ? result.error : "Unknown checkout error";
  ...
  const handleManageBilling = async () => {
    ...
    if (raw === "unauthorized" || raw.includes("unauthorized")) {
      ...
      raw.includes("portal_not_configured") ||
      raw.includes("not_configured")
    ) {
      msg =
        "Stripe Billing Portal is not set up yet. Activate it in your Stripe Dashboard → Settings → Billing → Customer portal.";
  ...
  const handleExportData = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `vouchedge-export-${Date.now()}.json`;
      a.click();
  ...
  const handleScheduleDeletion = async () => {
    if (
      !window.confirm(
        "Schedule account deletion? You have 30 days to cancel. Active subscriptions will be cancelled.",
      )
    )
      return;
    ...
    try {
      ...
      showToast(
        result.message ||
          `Deletion scheduled for ${formatDate(result.deletion_scheduled_at)}.`,
        "err",
      );
    } catch (err: any) {
  ...
  const handleResetClick = () => {
    if (
      !window.confirm(
        "Reset all local picks, slips, vouches, and profile data on this device?",
      )
    )
      return;
    onResetDatabase();
  ...
  return (
    <div
      className={`relative ve-page-shell min-h-0 min-w-0 overflow-x-hidden bg-ve-obsidian text-ve-flash ve-safe-bottom pb-24 md:pb-8 ${Z8_PAGE}`}
    >
      {/* Toast — clear mobile FAB cluster */}
      {toast && (
        <div
          className={`fixed bottom-[calc(5rem+env(safe-area-inset-bottom))] left-4 right-4 z-50 flex items-center gap-2.5 rounded-xl border px-4 py-3 text-sm font-medium shadow-2xl backdrop-blur sm:bottom-6 sm:left-auto sm:right-6 sm:max-w-sm ${
            toast.type === "ok"
      ...
      {/* Page header */}
      <div
        className={`glass-command border-b border-ve-fuse/40 bg-ve-obsidian/95 py-4 sm:py-5 ${Z8_PAGE_PAD_X}`}
      >
        <div className="mx-auto max-w-5xl">
          <div
            className={`flex items-center gap-2 text-xs font-medium text-white/40 ${Z8_LABEL}`}
          >
      ...
      <div className={`mx-auto max-w-5xl py-5 sm:py-8 ${Z8_PAGE_PAD_X}`}>
        ...
        <nav className="mb-5 lg:hidden" aria-label="Settings sections">
          ...
          <div
            className={`mt-3 flex items-center justify-between gap-3 rounded-xl ${Z8_PANEL_PREMIUM} p-3`}
          >
            <div className="min-w-0">
          ...
          <div className="min-w-0 flex-1">
            ...
            {activeTab === "account" && (
              <>
                <form onSubmit={handleProfileSave} className="space-y-8">
                  ...
                  <Section
                    title="Profile"
                    subtitle="This is your public identity on VouchEdge."
                  >
                    <div
                      className={`${Z8_SURFACE} flex flex-col gap-4 rounded-xl p-4 sm:flex-row sm:items-center sm:p-5`}
                    >
                  ...
                  <Section
                    title="General"
                    subtitle="Update your display name, username, and bio."
                  >
                      ...
                      <div className="grid gap-4 sm:grid-cols-2">
                        ...
                        <div className="space-y-1.5">
                          ...
                          <div className="flex rounded-lg border border-slate-700 bg-slate-900 focus-within:border-blue-500 focus-within:ring-1 focus-within:ring-blue-500/30">
                            <span className="flex items-center pl-3 text-sm text-slate-600">
                              @
                            </span>
                            <input
                              value={username}
                  ...
                  <Section
                    title="Social links"
                    subtitle="Connect your public profiles and channels."
                  >
                    <div className="grid gap-4 sm:grid-cols-2">
                      {[
                        {
                        ...
                        },
                        {
                          key: "discord",
                          label: "Discord",
                          placeholder: "server or username",
                          value: discord,
                          set: setDiscord,
                        },
                        {
            ...
            {activeTab === "billing" && (
              <div className="space-y-8">
                ...
                >
                  {/* Current plan row */}
                  <div
                    className={`${Z8_SURFACE} flex flex-col gap-4 rounded-xl p-4 sm:flex-row sm:items-center sm:justify-between sm:px-5 sm:py-4`}
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p className="text-sm font-medium text-white">
                          {PLAN_COPY[activeTier].title} plan
                        </p>
                        <p className="text-xs text-slate-500">
                          {billingStatus
                            ? `Renews ${formatDate(billingStatus.currentPeriodEnd)}`
                            : PLAN_COPY[activeTier].price +
                              (activeTier !== "BASIC" ? "/month" : " forever")}
                        </p>
                      </div>
                    </div>
                ...
                <Section
                  ...
                  subtitle="Update your card, view invoices, or cancel your subscription."
                >
            ...
            {activeTab === "notifications" && (
              <div className="space-y-8">
                <Section
                  title="Email"
                  subtitle="Control which emails VouchEdge sends to you."
                >
                  <div className="divide-y divide-slate-800 rounded-xl border border-slate-800">
                    <PrefRow
                      label="Account alerts"
                      detail="Security and billing notifications."
                    >
                      <Toggle checked={emailAlerts} onChange={setEmailAlerts} />
                    </PrefRow>
                    <PrefRow
                      label="Weekly recap"
                      detail="A summary of your picks, results, and activity each week."
                    >
                      <Toggle
                ...
                <Section
                  title="In-app"
                  subtitle="Push alerts and real-time updates inside the app."
                >
                  <div className="divide-y divide-slate-800 rounded-xl border border-slate-800">
                    <PrefRow
                      label="Push notifications"
                      detail="Parlay grading, HR board hits, and live game alerts."
                    >
                      <Toggle checked={pushAlerts} onChange={setPushAlerts} />
                    </PrefRow>
                    <PrefRow
                      label="Following alerts"
                      detail="Posts and activity from people you follow. Turned on automatically when you follow someone."
                    >
                      <Toggle
                        checked={followAlerts}
                        onChange={(value) => {
                          setFollowAlerts(value);
                          void persistNotificationPref({
                            follow_alerts_enabled: value,
                          });
                        }}
                      />
                    </PrefRow>
                    <PrefRow
                      label="Tailing alerts"
                      detail="When creators you tail lock slips or share parlays."
                    >
                      <Toggle
                        checked={tailAlerts}
                        onChange={(value) => {
                    ...
                    <PrefRow
                      ...
                      detail="Show your creator profile and stats on public leaderboards."
                    >
                      <Toggle
                    ...
                    <PrefRow
                      label="Reduce motion"
                      detail="Lower animation intensity across the interface."
                    >
                      <Toggle
            ...
            {activeTab === "privacy" && (
              <div className="space-y-8">
                <Section
                  title="Your data"
                  subtitle="Download a copy of everything VouchEdge holds about your account."
                >
                  <div className="flex flex-col gap-4 rounded-xl border border-slate-800 bg-slate-900/40 p-4 sm:flex-row sm:items-center sm:justify-between sm:px-5 sm:py-4">
                    <div className="min-w-0">
                      ...
                      <p className="mt-0.5 text-xs leading-relaxed text-slate-500">
                        Download your picks, parlays, profile, and activity as
                        JSON.
                      </p>
                    </div>
                    <VEButton
                ...
                <Section
                  title="Local data"
                  subtitle="Reset preview data stored on this device only."
                >
                  <div className="flex flex-col gap-4 rounded-xl border border-slate-800 bg-slate-900/40 p-4 sm:flex-row sm:items-center sm:justify-between sm:px-5 sm:py-4">
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-white">
                      ...
                      <p className="mt-0.5 text-xs leading-relaxed text-slate-500">
                        Clears picks, slips, vouches, and profile previews on
                        this browser.
                      </p>
                    </div>
                    <VEButton
                    ...
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between sm:gap-6">
                      <div className="min-w-0">
                        ...
                        <p className="mt-0.5 text-xs leading-relaxed text-red-400/70">
                          Schedules deletion with a 30-day grace period. Active
                          subscriptions will be cancelled. This cannot be
                          undone.
                        </p>
                      </div>
                      <VEButton
