...
import { useProfileFeedQuery } from "../hooks/queries/useFeedQuery";
import {
  Z8_LABEL,
  Z8_PAGE,
  Z8_PAGE_GAP,
...
export default function ProfilePage({
  ...
  const viewedProfileQuery = useQuery({
    queryKey: ["profile", "public", viewUserId],
    queryFn: () =>
      apiClient.get<Record<string, unknown>>(
        `/api/profile/${encodeURIComponent(viewUserId!)}`,
      ),
    enabled: Boolean(viewUserId && viewUserId !== user?.id),
    staleTime: 60_000,
  });
  const profile = useMemo(
    () =>
      viewedProfileQuery.data
        ? mapAuthMeToCreatorProof(viewedProfileQuery.data)
        : initialProfile,
    [initialProfile, viewedProfileQuery.data],
  );
  const socialStats = useProfileSocialStats(profileUserId);
  const profileActivityQuery = useProfileFeedQuery(
  ...
  const getActiveThemeData = () => {
    const themeToFind =
      profile.profileThemeId || profile.activeTheme || "cyber-blue";
          ...
          if (match) {
            return {
              ...
              avatarAnimationClass:
                match.avatarAnimationClass ||
                "animate-pulse border-sky-400 shadow-md",
              ...
              customAIPhrase:
                match.customAIPhrase || "🚀 CUSTOM THEME REVENUE INSTANCE LIVE",
            };
          }
        }
      } catch {
  ...
  if (!isOwnProfile && viewedProfileQuery.isLoading) {
    return (
      <div
        className={`${Z8_PAGE} ve-page-shell min-h-0 min-w-0 max-w-[1120px] mx-auto ${Z8_PAGE_PAD_X} ${Z8_PAGE_PAD_Y}`}
      >
        <div
          className={`${Z8_PANEL_PREMIUM} rounded-2xl p-6 text-sm text-white/65`}
        >
  ...
  if (!isOwnProfile && viewedProfileQuery.isError) {
    return (
      <div
        className={`${Z8_PAGE} ve-page-shell min-h-0 min-w-0 max-w-[1120px] mx-auto ${Z8_PAGE_PAD_X} ${Z8_PAGE_PAD_Y}`}
      >
  ...
  // Filter user's posts
  const activityPosts = isOwnProfile
    ? posts
    : (profileActivityQuery.data?.posts ?? []);
  const userPosts = activityPosts.filter(
    (p) =>
      (profileUserId != null && p.userId === profileUserId) ||
      p.username === profile.username,
  );

  // Filter user's results
  const userResults = activityPosts.filter(
    (p) =>
      p.postType === "RESULT" &&
      p.result &&
      ((profileUserId != null && p.userId === profileUserId) ||
  ...
  return (
    <ProfileThemeWrapper
      themeId={profile.profileThemeId || profile.activeTheme || "cyber-blue"}
    >
      <div
        className={`${Z8_PAGE} ve-page-shell min-h-0 min-w-0 max-w-[1120px] mx-auto overflow-x-hidden bg-ve-obsidian text-ve-flash ve-safe-bottom ${Z8_PAGE_PAD_X} ${Z8_PAGE_PAD_Y} ${Z8_PAGE_GAP}`}
      ...
      >
        {!isOwnProfile && onClearViewUser && (
          <header className="flex flex-wrap items-center justify-between gap-2">
            ...
            <button
              ...
              className="z8-control inline-flex min-h-11 items-center justify-center gap-2 border border-white/10 bg-black/30 px-4 font-mono text-[11px] font-bold uppercase tracking-wide text-white/70 hover:border-vouch-cyan/35 hover:text-white"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-8 space-y-6">
            {/* Main card */}
            <div
              className={`${Z8_PANEL_PREMIUM} rounded-2xl overflow-hidden relative transition-all duration-300 ${
                activeThemeData
                  ? activeThemeData.fontFamily || "font-sans"
                  : "font-sans"
              }`}
            ...
            >
              ...
              {profile.activeTheme === "google_cats" && (
                <>
                  ...
                  <div className="absolute -top-3.5 right-12 flex flex-col items-center pointer-events-none select-none z-30 transition-all hover:-top-1 group cursor-pointer">
                    <span
                      className="text-3xl filter drop-shadow-[0_2px_4px_rgba(0,0,0,0.6)] animate-bounce"
                      style={{ animationDuration: "3s" }}
                    >
                      🐱
                    </span>
                    <span className="text-[8px] bg-yellow-400 text-ve-surface-panel px-1.5 py-0.5 rounded font-black uppercase font-mono tracking-wider border border-slate-900 shadow leading-none scale-90">
                      G_KITTY
                    </span>
                  </div>
                  ...
                  <div className="absolute top-1/3 -left-3.5 flex items-center pointer-events-none select-none z-30 animate-pulse">
                    <span className="text-3xl rotate-45 filter drop-shadow-[0_2px_4px_rgba(0,0,0,0.5)]">
                      🐈
                    </span>
                  </div>
              ...
              <div
                className={`h-28 sm:h-32 bg-gradient-to-r relative border-b border-vouch-cyan/15 z-10 ${
                  activeThemeData
                    ? activeThemeData.coverBg ||
                      "from-sky-600/25 to-indigo-600/25"
                    : "from-sky-600/25 to-indigo-600/25"
                }`}
              >
                ...
                <div className="absolute -bottom-10 left-6 z-10">
                  ...
                  {profile.activeTheme === "google_cats" && (
                    <>
                      ...
                      <div className="absolute -left-3.5 top-6 font-mono font-black text-[#EA4335] text-xs pointer-events-none select-none z-10 rotate-12 animate-pulse">
                        🐾
                      </div>
                      <div className="absolute -right-3.5 top-6 font-mono font-black text-[#FBBC05] text-xs pointer-events-none select-none z-10 -rotate-12 animate-pulse">
                        🐾
                      </div>
                    </>
                  )}
              ...
              {avatarError && isOwnProfile && (
                <p className="px-6 pt-3 text-sm text-rose-200" role="alert">
                  {avatarError}
                </p>
              )}
              ...
              <div className="p-6 pt-12 space-y-4">
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
                  <div>
                    ...
                    <p
                      className={`${Z8_LABEL} text-white/40 mt-0.5 normal-case tracking-normal font-medium`}
                    >
                    ...
                    <div
                      className={`flex flex-wrap items-center gap-x-4 gap-y-1 mt-2.5 text-[11px] text-white/45 font-medium ${Z8_TABULAR}`}
                    >
                    ...
                    {activeThemeData?.customAIPhrase && (
                      <div className="mt-2.5 p-2 bg-slate-950/55 rounded-xl border border-slate-850/60 flex items-center gap-2 max-w-fit shadow-inner">
                        <span className="text-[8.5px] bg-indigo-950/50 text-indigo-400 font-mono font-extrabold px-1.5 py-0.5 rounded uppercase tracking-wide border border-indigo-900/30">
                          {activeThemeData.badge
                            ? activeThemeData.badge.toUpperCase()
                            : "THEM_AURA"}
                        </span>
                ...
                {isEditing ? (
                  <form
                    ...
                    className="space-y-3.5 bg-ve-graphite p-4 rounded-xl border border-slate-850"
                    id="profile-edit-subform"
                  >
                ...
                <div className="space-y-3.5 pt-1">
                  <h4
                    className={`${Z8_LABEL} text-white/55 flex items-center gap-1.5`}
                  >
                  ...
                  <div
                    className={`grid grid-cols-2 sm:grid-cols-4 gap-3 text-center text-xs ${Z8_TABULAR}`}
                  >
                    <div className={`${Z8_STAT_CHIP} p-3`}>
                      <p className={`${Z8_LABEL} text-white/35`}>
                        True Win Rate
                      </p>
                      <p className="font-mono text-base font-black text-vouch-cyan mt-1">
                    ...
                    <div className={`${Z8_STAT_CHIP} p-3`}>
                      ...
                      <p
                        className={`font-mono text-base font-black mt-1 ${profile.unitsNetProfit >= 0 ? "text-vouch-emerald" : "text-rose-400"}`}
                      >
                        {profile.unitsNetProfit >= 0 ? "+" : ""}
                        {profile.unitsNetProfit.toFixed(1)}U
                      </p>
                    </div>
                    <div className={`${Z8_STAT_CHIP} p-3`}>
                      <p className={`${Z8_LABEL} text-white/35`}>
                      ...
                      <p className="font-mono text-base font-black text-white mt-1">
                        {profile.totalPicks}
                      </p>
                    </div>
                    <div className={`${Z8_STAT_CHIP} p-3`}>
                      <p className={`${Z8_LABEL} text-white/35`}>Total Won</p>
                      <p className="font-mono text-base font-black text-vouch-emerald mt-1">
                        {profile.wonPicks}
                      </p>
                    </div>
                  </div>
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <span
                        className={`${Z8_LABEL} text-white/35 block leading-tight`}
                      >
                        Subscription Tier
                      </span>
                      <span className="font-extrabold text-white text-xs tracking-wide">
                        {profile.subscriptionTier === "SELLER_PRO"
                          ? "💎 SELLER PRO MONETIZED"
                          : profile.subscriptionTier === "GOLD"
                            ? "✨ VEDGE GOLD"
                            : "🛡️ VEdge BASIC PARTNER"}
                      </span>
                    </div>
                  </div>
                  <div className="text-right">
                    <span className={`${Z8_LABEL} text-white/35 block`}>
                      Status
                    </span>
                    <span className="text-vouch-emerald text-xs font-black font-mono">
                      {profile.totalPicks > 0
                        ? `${profile.totalPicks} PICKS TRACKED`
                        : "NO SETTLED PICKS"}
                    </span>
                  </div>
                </div>
                ...
                {isOwnProfile && !canEditHeader && (
                  <div className="flex items-start gap-2 rounded-xl border border-vouch-amber/20 bg-vouch-amber/5 px-3.5 py-2.5 text-[11px] text-white/55">
                    ...
                    <p>
                      Profile header themes are a{" "}
                      <span className="text-vouch-emerald font-bold">Gold</span>{" "}
                      perk.{" "}
                      <button
                        type="button"
                        onClick={() => onSectionChange?.("premium")}
            ...
            <div
              className={`${Z8_PANEL_PREMIUM} rounded-2xl p-3.5 flex items-start gap-2.5`}
            >
              <Info className="w-4 h-4 text-slate-400 mt-0.5 shrink-0" />
              <div className="text-[11px] text-slate-400 leading-relaxed font-semibold">
                <span className="text-slate-350 block mb-1">
                  TRACKING DISCLOSURE:
                </span>
                Performance metrics are calculated from settled results
                available to this profile. Unsettled picks and local-only drafts
                are excluded. Historical results do not guarantee future
                outcomes.
              </div>
            </div>
            ...
            <div className="space-y-4 pt-2" id="user-archived-posts-section">
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1 px-1">
                <h3 className="text-xs font-bold text-slate-200 uppercase tracking-widest flex items-center gap-2">
                  ...
                  {isOwnProfile ? "Your" : "Shared"} Stakes & Ledger Posts ({userPosts.length})
                </h3>
              ...
              {profileActivityQuery.isLoading && !isOwnProfile ? (
                ...
                <div className="p-12 text-center bg-ve-storm rounded-2xl border border-slate-850 space-y-3">
                  ...
                  <div className="space-y-1">
                    <p className="text-xs text-slate-300 font-bold uppercase font-mono">
                      No Posts Published Yet
                    </p>
                    <p className="text-[11px] text-slate-550 max-w-sm mx-auto text-slate-450 leading-relaxed">
                      You haven't logged any sports analyses, result
                      settlements, or parlay slips to the feed yet. Use the
                      composer on the Home Feed or the custom Vouch visual
                      generator to make your mark!
                    </p>
                  </div>
                </div>
              ) : (
          ...
          <div className="lg:col-span-4 space-y-6">
            <div
              className={`${Z8_PANEL_PREMIUM} rounded-2xl p-4.5 space-y-4 shadow-xl relative animate-slide-up`}
              id="profile-performance-graphs-card"
            >
              ...
              {(() => {
                const graphRows = profileDays
                  .map((day) => {
                    ...
                    const dayOutcomes = userResults.filter(
                      (r) => getLocalYMD(r.timestamp) === ymd,
                    );
                    const won = dayOutcomes.filter(
                      (r) => r.result?.status === "WON",
                    ).length;
                    const lost = dayOutcomes.filter(
                      (r) => r.result?.status === "LOST",
                    ).length;
                ...
                if (!hasGraphData) {
                  return (
                    <div
                      className={`${Z8_PANEL_PREMIUM} rounded-2xl p-5 text-center`}
                    >
                      <p className="text-xs font-black uppercase tracking-widest text-slate-300">
                        Start tracking picks to unlock your performance graph.
                      </p>
                      <p className="mt-2 text-[11px] text-slate-500">
                        Settled wins and losses will automatically populate your
                        profile portfolio.
                      </p>
                    </div>
                  );
            ...
            <div
              className={`hidden md:block ${Z8_PANEL_PREMIUM} rounded-2xl p-4.5 space-y-4 shadow-xl relative animate-slide-up`}
              id="profile-my-outcomes-sidemenu-card"
            >
              <div className="flex items-center justify-between border-b border-slate-850 pb-3">
              ...
              <p className="text-[11px] text-slate-400 leading-relaxed font-semibold">
                Settled results grouped by day from the profile record currently
                available.
                <strong className="text-slate-300">
                  {" "}
                  Hover over a date
                </strong>{" "}
                to inspect its recorded outcomes and unit result.
              </p>

              <div
                className="space-y-2 relative"
                id="private-ledgers-days-rows"
              >
                {profileDays.map((day) => {
                  const ymd = getLocalYMD(day);
                  ...
                  const wonCount = dayOutcomes.filter(
                    (r) => r.result?.status === "WON",
                  ).length;
                  const lostCount = dayOutcomes.filter(
                    (r) => r.result?.status === "LOST",
                  ).length;
                  const totalPlayed = wonCount + lostCount;
                  const winRate =
                    totalPlayed > 0 ? (wonCount / totalPlayed) * 100 : null;
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        {hasPlays ? (
                          ...
                          <span className="text-[9px] font-mono text-slate-600 uppercase font-black tracking-wider">
                            No Outcomes
                          </span>
                        )}
                      </div>
                        ...
                        <div className="absolute left-[-20px] right-[-20px] lg:left-auto lg:right-0 lg:w-[280px] top-[102%] z-50 bg-slate-950 border border-slate-800 p-3.5 rounded-2xl shadow-2xl space-y-3 animate-fadeIn">
                          ...
                          {hasPlays ? (
                            <div className="space-y-3">
                              ...
                              <div className="grid grid-cols-3 gap-2 text-center text-[10px]">
                                <div className="bg-ve-storm p-1.5 rounded-md border border-slate-850">
                                  <p className="text-slate-400 uppercase font-black tracking-wider">
                                    Winrate
                                  </p>
                                ...
                                <div className="bg-ve-storm p-1.5 rounded-md border border-slate-850">
                                  ...
                                  <p className="text-slate-200 font-mono font-bold text-xs mt-0.5">
                                ...
                                <div className="bg-ve-storm p-1.5 rounded-md border border-slate-850">
                                  ...
                                  <p
                                    className={`font-mono font-bold text-xs mt-0.5 ${netProfit >= 0 ? "text-emerald-400" : "text-rose-455"}`}
                                  >
                                    {netProfit >= 0 ? "+" : ""}
                                    {netProfit.toFixed(1)}U
                                  </p>
                                </div>
                              </div>
                              ...
                              <div className="space-y-1.5 max-h-[140px] overflow-y-auto no-scrollbar">
                                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest block pl-0.5">
                                  Settled Markets
                                </p>
                                {dayOutcomes.map((item, idx) => (
                                  <div
                                    key={idx}
                                    className="flex justify-between items-center text-[10px] bg-slate-900/40 p-1.5 rounded-lg border border-slate-850"
                                  >
                                    <span className="text-slate-300 font-medium truncate max-w-[140px]">
                                      {item.result?.marketName}
                                    </span>
                                    <span
                                      className={`font-mono font-black px-1.5 rounded text-[9px] ${
                                        item.result?.status === "WON"
                            ...
                            <p className="text-[10px] text-slate-500 text-center font-mono py-2 leading-relaxed">
                              No ledger outcomes recorded for this date. Settle
                              plays in your feed to populate.
                            </p>
                          )}
                        </div>
                      )}
        ...
        {showShareModal && (
          <div
            ...
            id="profile-share-modal-overlay"
          >
            <ProfileShareCard
              profile={profile}
