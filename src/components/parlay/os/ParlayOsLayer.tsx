...
function SavedParlaysView({ savedSlips }: { savedSlips: any[] }) {
...
export default function ParlayOsLayer({
  ...
  useEffect(() => {
    ...
    const handleSaveParlay = useCallback(async () => {
    ...
    try {
      const data = await apiClient.post('/api/v3/parlays/save', rawSlip);
      
      if (data && data.ok) {
        replaceOptimisticSlip(optimisticId, data.parlay || data.data || data);
        notify({
          kind: "success",
          title: "Parlay Saved!",
