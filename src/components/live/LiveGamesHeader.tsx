...
interface LiveGamesHeaderProps {
  ...
  finalCount: number;
  filterTab: LiveGamesFilterTab;
  onFilterChange: (tab: LiveGamesFilterTab) => void;
}

const FEED_COPY: Record<LiveGamesFeedState, string> = {
  live: 'Streaming',
...
const TABS: Array<{
  key: LiveGamesFilterTab;
  label: string;
  tone: 'cyan' | 'rose' | 'sky' | 'emerald';
}> = [
...
export function LiveGamesHeader({
  onRefresh,
  isSyncing,
  feedState,
  feedNote,
  ...
  filterTab,
  onFilterChange,
}: LiveGamesHeaderProps) {
  const counts: Record<LiveGamesFilterTab, number> = {
    all: totalCount,
    live: liveCount,
