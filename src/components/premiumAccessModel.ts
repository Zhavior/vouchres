...
interface BuildPremiumAccessModelInput {
...
export function buildPremiumAccessModel({
  profileTier,
  billingStatus,
  billingSourceState,
}: BuildPremiumAccessModelInput): PremiumAccessModel {
  ...
  if (billingSourceState === 'checking') {
    return {
      ...
      billingDetail: 'VouchEdge is waiting for the server before confirming subscription status.',
      hasPaidAccess,
