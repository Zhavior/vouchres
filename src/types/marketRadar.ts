...
export interface MlbPitcherKResearch {
  subjectId: string;
  subject: string;
  team: string;
  opponent: string;
...
export interface MlbStolenBaseResearch {
  subjectId: string;
  subject: string;
  team: string;
  opponent: string;
...
export interface MlbMarketRadarResponse {
  date: string;
  generatedAt: string;
  provider: { id: 'mlb_stats_api'; status: 'live'; eventCount: number; signalCount: number };
