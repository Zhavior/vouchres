...
export async function getHitterStats(playerId: number): Promise<HitterStats> {
  return hitterCache.getOrSet(`hitter_season:${playerId}:${SEASON}`, async () => {
    let season: HitterSeasonStats | null = null;
    let recentGames: RecentGame[] = [];

    try {
      season = await getSingleSeasonHitterStats(playerId, SEASON);
    } catch (err) {
      console.warn(`[statsClient] current-season hitter stats failed for ${playerId}:`, (err as Error).message);
    }
