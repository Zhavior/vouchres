...
export function buildParlayLegSettledNotification(args: {
  ...
  return {
    ...
    title: "Leg Settled",
    message: `Leg ${args.legIndex} (${args.selection}) was settled: ${args.status}.`,
    metadata: {
      parlayId: args.parlayId,
...
export async function createParlayLegSettledNotification(args: {
  ...
  const payload = buildParlayLegSettledNotification(args);
  const result = await createNotification({
    userId: args.userId,
    ...payload,
  });
  console.log(`[notifications] parlay leg settled parlay=${args.parlayId} leg=${args.legIndex} status=${args.status} created=${result.created}`);
  return result;
}
