...
export default function SocialHubZ8() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    channel
      ...
      .subscribe(async (status) => {
        ...
        // V3 Architecture: UI posts intent to Outbox, Backend worker broadcasts presence
        void apiClient.post('/api/social/v3/presence', {
          state: 'online',
          page: activeTab,
        }).catch(() => {});
      });
    ...
    return () => {
      disposed = true;
      // Do not untrack directly, backend manages presence TTL
        ...
        <nav className="space-y-1">
          {(['circle', 'messages'] as const).map((tab) => (
            <button
              ...
              className={`w-full flex items-center justify-between rounded-xl px-4 py-3 text-sm font-bold transition-colors ${
                activeTab === tab
                  ? 'bg-vouch-emerald/15 text-vouch-emerald'
                  : 'text-white/45 hover:bg-white/[0.04] hover:text-white'
              }`}
            >
              <span>{tab === 'circle' ? 'Circle' : 'Messages'}</span>
              {tab === 'messages' && dm.unreadCount > 0 ? (
                <span className="rounded-full bg-vouch-emerald px-1.5 py-0.5 text-[10px] leading-none text-obsidian-900">
                  {dm.unreadCount}
                </span>
              ) : null}
            </button>
